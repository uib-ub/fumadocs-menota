...
In [ch. 3.6](/handbook/v3/HB3_ch3#sec3.6) and [ch. 5.3](/handbook/v3/HB3_ch5#sec5.3), we suggested that the word, `<w>{:xml}`, is a basic unit in any transcription. Each `<w>{:xml}` element in a manuscript text can easily be supplied with information about the dictionary entry and the grammatical analysis of the word in question. We recommend that this information is provided by two attributes, `@lemma{:sh}` for the dictionary entry and `@me:msa{:sh}` for the grammatical form:

<div className="table-abbr">
| Element & attributes | Contents |
|-----|-----|
| `<w>{:xml}` | Delimits a grammatical word. |
...
<tr>
<td>han`<am>{:xml}`&bar;`</am>{:xml}`</td>
</tr>
<tr>
<td>h`<am>{:xml}`&bar;`</am>{:xml}`</td>
</tr>
<tr>
<td>h`<am>{:xml}`&bar;`</am>{:xml}`n</td>
</tr>
<tr>
<td>ha&nscap;</td>
</tr>
...
<tr>
<td>h`<am>{:xml}`&bar;`</am>{:xml}`s</td>
</tr>
<tr>
<td>h`<am>{:xml}`&bar;`</am>{:xml}`ſ</td>
</tr>
<tr>
<td>honum</td>
...
In cases where a graphic word is included partially or completely in the element `<unclear>{:xml}` this can be encoded within the element `<w>{:xml}` and be related to the attribute `@lemma{:sh}`.
```xml
<w lemma="svá">  
  <choice>
...
Text included within the element `<supplied>{:xml}` is not lemmatised. The following example shows how a character, word or phrase that has been supplied is encoded with the element `<w>{:xml}`, but without any `@lemma{:sh}` attribute as the text (in whole or in part) is not transcribed from the manuscript itself.
```xml
<w> 
  <choice>
...
In this encoding, we use the `<seg>{:xml}` element as a container in a similar way to the encoding of words which are written together, cf. [ch. 5.3.2](/handbook/v3/HB3_ch5#sec5.3.2). The value `"nb"{:js}` (for “no break”) indicates that there is no space between the parts in the `<w>{:xml}` element.
...
In marginal cases, there can be two or even three enclitics, such as “vilkat” for “vil [e]k at” ‘I will not’ and “bjargigak” for “bjargi [e]k a[t] [e]k” ‘I do not I save’ (_Hávamál_, st. 149). In the encoding, each word part should be rendered in a separate `<w>{:xml}` element in the phonetic form it actually has:
```xml
<seg type="enc">
...
**Roman numerals**. Roman numerals are frequent in Medieval Nordic texts, and should be encoded as numbers using the `<num>{:xml}` element, e.g. `<num>{:xml}``<w>{:xml}`.iv.`</w>{:xml}``</num>{:xml}`. There will be no `@lemma{:sh}` attribute for Roman numerals as part of the `<w>{:xml}` element, but they may receive `@type{:sh}` and `@value{:sh}` attributes as part of the `<num>{:xml}` element. For details, see [ch. 5.7.1](/handbook/v3/HB3_ch5#sec5.7.1).
...
Thanks to the `<seg>{:xml}` encoding, the two constituents can be displayed as a single graphical word, “þótt”, in an encoded text.

## 11.7 Lemmatisation of non-Nordic material [#sec11.7]

The dominant language in a transcription should be specified as an attribute to the `<text>{:xml}` element. For a Menota transcription, that will typically be one of the Medieval Nordic languages. In this example, the text is specified as Swedish (“swe”):
```xml
<text xml:lang="swe">
...
If there is a lengthy passage in another language, the attribute can also be given at a higher level in the encoding, e.g. to a `<div>{:xml}` element.

All `@xml:lang{:sh}` attributes should be defined in the header. This is part of the `<profileDesc>{:xml}` element, which must contain a list of all languages referred to in the encoded text. We recommend this standard set of Nordic languages plus Greek and Latin:
```xml
<langUsage> 
