...
In [ch. 5.3.2](HB3_ch5.xml#sec5.3.2) above the use of `<w>{:xml}` for the encoding of graphic words and information concerning their description is treated. Note the use of entities for special characters, such as ` &fins;` and `&nscap;`, or abbreviations such as `&bar;`. These are described in [ch. 5](HB3_ch5.xml).
