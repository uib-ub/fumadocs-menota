...
| &ensp;&ensp;&ensp;`@lemma` | States the lemma (lexical entry) of the word. |
| &ensp;&ensp;&ensp;`@me:msa` | States the grammatical (morphosyntactical) form of the word. |
</div>
...
In cases where a graphic word is included partially or completely in the element `<unclear>{:xml}` this can be encoded within the element `<w>{:xml}` and be related to the attribute `@lemma`.
```xml
<w lemma="svá">  
  <choice>
...
Text included within the element `<supplied>{:xml}` is not lemmatised. The following example shows how a character, word or phrase that has been supplied is encoded with the element `<w>{:xml}`, but without any `@lemma` attribute as the text (in whole or in part) is not transcribed from the manuscript itself.
```xml
<w> 
  <choice>
...
In this encoding, we use the `<seg>{:xml}` element as a container in a similar way to the encoding of words which are written together, cf. [ch. 5.3.2](HB3_ch5.xml#sec5.3.2). The value `"nb"` (for “no break”) indicates that there is no space between the parts in the `<w>{:xml}` element.
...
**Roman numerals**. Roman numerals are frequent in Medieval Nordic texts, and should be encoded as numbers using the `<num>{:xml}` element, e.g. `<num>{:xml}``<w>{:xml}`.iv.`</w>{:xml}``</num>{:xml}`. There will be no `@lemma` attribute for Roman numerals as part of the `<w>{:xml}` element, but they may receive `@type` and `@value` attributes as part of the `<num>{:xml}` element. For details, see [ch. 5.7.1](HB3_ch5.xml#sec5.7.1).
...
All `@xml:lang` attributes should be defined in the header. This is part of the `<profileDesc>{:xml}` element, which must contain a list of all languages referred to in the encoded text. We recommend this standard set of Nordic languages plus Greek and Latin:
```xml
<langUsage> 
