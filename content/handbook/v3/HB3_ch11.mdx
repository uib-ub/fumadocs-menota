...
| han`<am>{:xml}`&bar;`</am>{:xml}` |
| h`<am>{:xml}`&bar;`</am>{:xml}` |
| h`<am>{:xml}`&bar;`</am>{:xml}`n |
| ha&nscap; |
| hans | hann | Genitive |
| hanſ |
| h`<am>{:xml}`&bar;`</am>{:xml}`s |
| h`<am>{:xml}`&bar;`</am>{:xml}`ſ |
| honum | hann | Dative |
| honom |
| h`<am>{:xml}`&bar;`</am>{:xml}`m |


In [ch. 5.3.2](HB3_ch5.xml#sec5.3.2) above the use of `<w>{:xml}` for the encoding of graphic words and information concerning their description is treated. Note the use of entities for special characters, such as ` &fins;{:xml}` and `&nscap;{:xml}`, or abbreviations such as `&bar;{:xml}`. These are described in [ch. 5](HB3_ch5.xml).
