...
In [ch. 3.6](HB3_ch3.xml#sec3.6) and [ch. 5.3](HB3_ch5.xml#sec5.3), we suggested that the word, `<w>{:xml}`, is a basic unit in any transcription. Each `<w>{:xml}` element in a manuscript text can easily be supplied with information about the dictionary entry and the grammatical analysis of the word in question. We recommend that this information is provided by two attributes, `@lemma{:sh}` for the dictionary entry and `@me:msa{:sh}` for the grammatical form:
...
In [ch. 5.3.2](HB3_ch5.xml#sec5.3.2) above the use of `<w>{:xml}` for the encoding of graphic words and information concerning their description is treated. Note the use of entities for special characters, such as ` &fins;{:xml}` and `&nscap;{:xml}`, or abbreviations such as `&bar;{:xml}`. These are described in [ch. 5](/handbook/v3/HB3_ch5).
...
In this encoding, we use the `<seg>{:xml}` element as a container in a similar way to the encoding of words which are written together, cf. [ch. 5.3.2](HB3_ch5.xml#sec5.3.2). The value `"nb"{:js}` (for “no break”) indicates that there is no space between the parts in the `<w>{:xml}` element.
...
On balance, we recommend encoding the relative particle simply as a subjunction, “xCS”. See the discussion in [Haugen and Øverland 2014](HB3_literature.xml#Guidelines2014), p. 38.
...
On the whole, Old Norse grammars and dictionaries comply with the traditional eight word classes (parts of speech), i.e. nouns, pronouns, verbs, adjectives, adverbs, prepositions, conjunctions and interjections. We suggest that a newer set of word classes should be used, in line with what is now the standard for e.g. Modern Norwegian, cf. [Norsk referansegrammatikk (1997)](HB3_literature.xml#NorskRef1997) by Jan Terje Faarlund, Svein Lie and Kjell Ivar Vannebo. This question is discussed at some length in the guidelines for the Menotec project, [Haugen and Øverland 2014](HB3_literature.xml#Guidelines2014), p. 13&ndash;40.
...
As stated above, we recommend that words are mapped to a single word class, even if they have rather diverse syntactical properties. The table below contains some of the problematic words, organised by word classes. The list is based on [Haugen and Øverland 2014](HB3_literature.xml#Guidelines2014), pp. 23&ndash;40. Many of the words below are discussed in some detail in these guidelines.
