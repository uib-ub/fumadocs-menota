...
Text included within the element `<supplied>{:xml}` is not lemmatized. The following example shows how a character, word or phrase that has been supplied is encoded with the element `<w>{:xml}`, but without any `@lemma{:sh}` attribute as the text (in whole or in part) is not transcribed from the manuscript itself.
```xml
<w> 
  <choice>
...
In this encoding, we use the `<seg>{:xml}` element as a container in a similar way to the encoding of words which are written together, cf. [ch. 5.3.2](/handbook/v3/HB3_ch5#sec5.3.2).
...
In a multi-level encoding, the stylesheet should not display any space between the main word and the enclitic on the `<me:facs>{:xml}` and `<me:norm>{:xml}` levels, but a space should be displayed on the `<me:norm>{:xml}` level. See the example in [ch. 5.3.2](/handbook/v3/HB3_ch5#sec5.3.2) above.
...
**Roman numerals**. Roman numerals are frequent in Medieval Nordic texts, and should be encoded as numbers using the `<num>{:xml}` element, e.g. `<num>{:xml}`.iv.`</num>{:xml}`. They should not be lemmatised. Cf. the discussion in [ch. 5.7](/handbook/v3/HB3_ch5#sec5.7) above.
...
The dominant language in a transcription should be specified as an attribute to the `<text>{:xml}` element. For a Menota transcription, that will typically be one of the Medieval Nordic languages. In this example, the text is specified as Old Swedish (“osw”):
```xml
<text xml:lang="osw">
