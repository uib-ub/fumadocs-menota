...
Text included within the element <E>supplied</E> is not lemmatized. The following example shows how a character, word or phrase that has been supplied is encoded with the element <E>w</E>, but without any <Att>lemma</Att> attribute as the text (in whole or in part) is not transcribed from the manuscript itself.
```xml
<w> 
  <choice>
...
This means that the forms that are not marked will not be included in the searchable database under the category <Att>lemma</Att>. 
