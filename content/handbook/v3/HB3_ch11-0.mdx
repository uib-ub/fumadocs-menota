...
In a multi-level encoding, the stylesheet should not display any space between the main word and the enclitic on the `<me:facs>{:xml}` and `<me:norm>{:xml}` levels, but a space should be displayed on the `<me:norm>{:xml}` level. See the example in [ch. 5.3.2](HB3_ch5.xml#sec5.3.2) above.11.3.2.12 Government
