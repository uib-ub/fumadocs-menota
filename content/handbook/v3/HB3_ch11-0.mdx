...
| &ensp;&ensp;&ensp;`@me:msa` | States the grammatical (morphosyntactial) form of the word. |
</div>
...
Text included within the element `<supplied>{:xml}` is not lemmatized. The following example shows how a character, word or phrase that has been supplied is encoded with the element `<w>{:xml}`, but without any `@lemma` attribute as the text (in whole or in part) is not transcribed from the manuscript itself.
```xml
<w> 
  <choice>
