...
In this encoding, we use the `<seg>{:xml}` element as a container in a similar way to the encoding of words which are written together, cf. [ch. 5.3.2](HB3_ch5.xml#sec5.3.2).
...
In a multi-level encoding, the stylesheet should not display any space between the main word and the enclitic on the `<me:facs>{:xml}` and `<me:norm>{:xml}` levels, but a space should be displayed on the `<me:norm>{:xml}` level. See the example in [ch. 5.3.2](HB3_ch5.xml#sec5.3.2) above.
...
**Roman numerals**. Roman numerals are frequent in Medieval Nordic texts, and should be encoded as numbers using the `<num>{:xml}` element, e.g. `<num>{:xml}`.iv.`</num>{:xml}`. They should not be lemmatised. Cf. the discussion in [ch. 5.7](HB3_ch5.xml#sec5.7) above.
