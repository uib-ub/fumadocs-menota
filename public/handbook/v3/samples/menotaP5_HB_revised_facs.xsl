...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:me="http://www.menota.org/ns/1.0">
<!-- This style sheet has been written by Vemund Olstad, Aksis, University of Bergen, for the Menota TVB project, http://www.hit.uib.no/menota/TVB. First version made for the research group Editing Medieval Manuscripts, Centre for Advanced Study, Oslo, 2000-2001. -->
...
<xsl:template match="tei:lb">
 ...
 <xsl:choose>
    ...
    <xsl:when test="attribute::n">
        <!-- Test -->
        <!--<xsl:if test="following::tei:add[1][attribute::place='supralinear'][following::tei:lb[1][attribute::n=$nextline]][ancestor::tei:lg[attribute::xml:id=$linegroup]][not(ancestor::me:dipl or ancestor::me:norm)]">
            <xsl:element name="BR"></xsl:element>
            <xsl:for-each select="following::tei:add[attribute::place='supralinear'][following::tei:lb[1][attribute::n=$nextline]][ancestor::tei:lg[attribute::xml:id=$linegroup]]">
                <xsl:apply-templates select="self::tei:add[attribute::place='supralinear'][following::tei:lb[1][attribute::n=$nextline]]" mode="supra"/>
            </xsl:for-each>
        </xsl:if>-->
        <!-- Slutt test -->
    	<xsl:element name="BR"></xsl:element>
...
<!-- Since spaces inside p elements are stripped from the file (see general comment at the start of the stylesheet), a function has been created within this template to generate spaces after individual words, but not before punctuation marks. -->    
    ...
    <xsl:template match="tei:w | tei:num | tei:name | tei:persName | tei:orgName">
        <xsl:choose>
            <xsl:when test="preceding-sibling::*[1][self::me:punct/ancestor::place='rightlocation']">
                <!-- 2008.06.16 TMB: If it is a punct with rigth localation, do not add a space. -->
            </xsl:when>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:choose>
                    ...
                    <xsl:when test="following-sibling::*[1][self::tei:w] and ancestor::tei:seg[attribute::type='nb'] and not(contains($visning, 'norm'))"></xsl:when> <!-- 2008.06.09 TMB: If it is contain in a seg which indicates that there should be no spaces between the words. -->
    ...
    <xsl:template match="me:punct | punct">
        <xsl:choose>
        ...
        <xsl:choose>
            ...
            <xsl:when test="contains(attribute::place, 'margin')">
                ...
                <xsl:choose>
                    <xsl:when test="$visning='facs'">
                        <xsl:choose>
                            <!--<xsl:when test="attribute::place='margin-top'">
                                <xsl:element name="SPAN">
                                    <xsl:attribute name="style">color: gray; font-size: 10pt; font-family: Georgia;</xsl:attribute>
                                    <xsl:value-of select="$marginal"/>
                                </xsl:element>
                            </xsl:when>
                            <xsl:when test="attribute::place='margin-bottom'">
                                <xsl:element name="SPAN">
                                    <xsl:attribute name="style">color: gray; font-size: 10pt; font-family: Georgia;</xsl:attribute>
                                    <xsl:value-of select="$marginal"/>
                                </xsl:element>
                            </xsl:when>-->
                            <!--<xsl:when test="attribute::place='margin-left'">
                                <xsl:element name="SPAN">
                                    <xsl:attribute name="style">color: gray; font-size: 10pt; font-family: Georgia;</xsl:attribute>
                                    <xsl:value-of select="$marginal"/>
                                </xsl:element>
                            ...
                            <xsl:when test="attribute::place='margin-right'">
                                ...
                                <xsl:element name="DIV">
                                    ...
                                    <!--<xsl:element name="SPAN">
                                        <xsl:attribute name="style">color: gray; font-family: Georgia;</xsl:attribute>
                                        <xsl:text>[</xsl:text>
                                        <xsl:value-of select="$marginal"/>
                                        <xsl:text>] </xsl:text>
                                    </xsl:element>-->
                            ...
                            <xsl:otherwise>
                                <!--<xsl:element name="SPAN">
                                    <xsl:attribute name="style">color: gray; font-size: 10pt; font-family: Georgia;</xsl:attribute>
                                    <xsl:if test="not(contains(attribute::place, 'left'))">
                                        <xsl:value-of select="$marginal"/>
                                    </xsl:if>
                                    <xsl:if test="contains(attribute::place, 'left')">
                                        <xsl:value-of select="$marginal"/>
                                    </xsl:if>
                                </xsl:element>-->
                                <!--<xsl:element name="DIV">
                                    <xsl:attribute name="style">display: inline; float: right; margin-right: 300px; clear: none; border: 1px solid black; width: 100px; font-size: 12pt; line-height: 135%</xsl:attribute>
                                    <xsl:element name="SPAN">
                                        <xsl:attribute name="style">color: gray; font-family: Georgia;</xsl:attribute>
                                        <xsl:text>[</xsl:text>
                                        <xsl:value-of select="$marginal"/>
                    ...
                    <xsl:otherwise>
                        <!-- Dette er den originale visningen -->
                        <!--<xsl:text>&#x2E1D;</xsl:text>-->
                        <xsl:text>&#x2E0D;</xsl:text>
                        <xsl:apply-templates/>
                        <xsl:text>&#x2E0C;</xsl:text>
                ...
                <!-- Slutt test -->
                <!-- ! HB 2014.03.25: Changed ´to &#x2E0C; and ` to &#x2E0D; in section below -->
            </xsl:when>
            <xsl:otherwise>
                <xsl:text>&#x2E0C;</xsl:text>
                <xsl:apply-templates/>
...
<xsl:template match="tei:unclear">
    ...
    <!-- This template is used to output an asterisk in front of corr-elements. New 2008-10-22, VO -->
...
<xsl:template match="me:expunged">
    ...
    <xsl:template match="tei:anchor">
        <xsl:choose>
            <xsl:when test="contains($visning, 'norm') or contains($visning, 'dipl')">
                ...
                <xsl:variable name="category"><xsl:value-of select="preceding::me:textSpan[1][attribute::spanTo=$ref]/attribute::category"/></xsl:variable>
                <xsl:choose>
                    <!-- Have to look up the corresponding start element to see what kind of anchor element this is. -->
                    <xsl:when test="$category='add'">
                        <xsl:text>´</xsl:text>
                    </xsl:when>
                    <xsl:when test="$category='corr'">
                        <xsl:text></xsl:text>
                    </xsl:when>
                    <xsl:when test="$category='del'">
                        <xsl:text>-|</xsl:text>
                    </xsl:when>
                    <xsl:when test="$category='damage'">
                        <xsl:text>]</xsl:text>
                    </xsl:when>
                    <xsl:when test="$category='supplied'">
                        <!--<xsl:text>&gt;</xsl:text>-->
                        <xsl:text>&#x27E9;</xsl:text>
                    </xsl:when>
                </xsl:choose>
    ...
    <xsl:template match="tei:c">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:element name="SPAN">
                    <!--<xsl:attribute name="style">background-color: #EBEBEB; color: #FF0000;</xsl:attribute>-->
                    <xsl:apply-templates/>
                </xsl:element>
            </xsl:otherwise>
        </xsl:choose>
